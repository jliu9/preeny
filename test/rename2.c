...
void test_rename3() {
  ...
  rt = close(fd);
  CHECK_ACTUAL_EXPECT_VALUE(closefsrc, rt, 0);
  fd = open(test_dst_name, O_CREAT, common_mode);
  CHECK_COND(createfdst, fd > 0);
  rt = close(fd);
  CHECK_ACTUAL_EXPECT_VALUE(closefdst, rt, 0);
  assert(g_pass);
  ...
  CHECK_ACTUAL_EXPECT_VALUE(statfdst, rt, 0);
  dst_ino = stbuf.st_ino;
  fprintf(stdout, "src_ino:%ld dst_ino:%ld\n", src_ino, dst_ino);
  CHECK_COND(inoTheSame, src_ino == dst_ino);
  assert(g_pass);
}
...
void test_rename4() {
  ...
  rt = mkdir(test_src_name, common_mode);
  CHECK_ACTUAL_EXPECT_VALUE(mkdsrc, rt, 0);
  rt = mkdir(test_dst_name, common_mode);
  CHECK_ACTUAL_EXPECT_VALUE(mkdsrc, rt, 0);
  assert(g_pass);
